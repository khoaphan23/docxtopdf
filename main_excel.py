"""
Excel → PDF (clean main compatible with new ConverterUI)
"""
...
from src.converters.excel_to_pdf import ExcelConverter
from src.io.file_handler import FileHandler

SUPPORTED_EXTENSIONS_EXCEL: Tuple[str, ...] = (".xls", ".xlsx")


class ExcelApp:
    """
    Ứng dụng Excel → PDF gọn – chuẩn, tương thích ConverterUI mới (không dùng `callbacks=`).
    ...
    def _on_select(self) -> None:
        """Chọn file Excel (ưu tiên hàm trong FileHandler; fallback nếu không có)."""
        try:
            # prefer project-specific selector if present
            if hasattr(self.fh, "select_excel_file"):
                path = self.fh.select_excel_file(parent=self.root)
            else:
            ...
            if not path:
                # user cancelled
                if self.ui:
                    self.ui.update_status("✅ Sẵn sàng - Hãy chọn file Excel để bắt đầu", 0)
                return

            p = Path(path)

            # validate nếu FileHandler có sẵn validate_excel_file
            if hasattr(self.fh, "validate_excel_file"):
                ok, msg = self.fh.validate_excel_file(p)  # type: ignore[attr-defined]
                if not ok:
                    if self.ui:
    ...
    def _on_convert(self) -> None:
        """Chuyển file Excel → PDF: lưu thẳng vào Downloads (tránh bước 'tải xuống')."""
        if not self.selected_file:
            if self.ui:
                self.ui.alert_warning("Chưa chọn file", "Vui lòng chọn file Excel trước khi chuyển!")
            return
    ...
    def run(self) -> None:
        ...
        self.ui = ConverterUI(
            ...
            title_text="📊 Excel to PDF Converter",
            select_button_text="Chọn file Excel",
        ...
        if self.ui:
            self.ui.update_status(f"{'✅' if ok else '⚠️'} {info} - Hãy chọn file Excel", 0)
